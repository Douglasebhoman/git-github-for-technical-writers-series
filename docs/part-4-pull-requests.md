...
Documentation can be understood as versioned state, a structured snapshot of documentation at a specific point in time. Every commit creates a new state. But not every state should become authoritative.
...
This page examines Pull Requests not as conversation tools, but as structural control systems that determine which documentation state becomes production documentation.
...
Branch protection rules are configured in your repository under **Settings**, then **Branches**, then **Branch protection rules**.

With enforcement in place:

- Required reviewers must approve before merge is permitted
...
That gate protects the integrity of the production documentation state. Disciplined teams treat documentation PRs with the same seriousness as code PRs. Both modify production-facing artefacts and both carry the same responsibility.
...
- Enforcement through branch protection rules, required reviewers, and CI checks is what makes a Pull Request function as governance rather than optional ceremony.
...
Move to [Part 5: CI/CD for Technical Writers](part-5-cicd-pipelines.md) to continue.
